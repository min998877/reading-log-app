...
def restore_from_backup(uploaded_file) -> bool:
    ...
    try:
        ...
        # 현재 DB 백업 (안전을 위해)
        backup_before_restore = f"{db_path}_before_restore_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db"
        shutil.copy2(db_path, backup_before_restore)
        
        # 업로드된 파일을 임시 DB로 저장
        temp_db = f"{db_path}_temp.db"
        ...
        temp_db = f"{get_personal_db_path()}_temp.db"
...
def get_personal_db_path():
    ...
    db_name = f"{db_dir}/reading_log_{user_id}.db"
    return db_name
...
# 중요 안내 메시지 - 매우 눈에 띄게 표시
if 'is_new_user' in st.session_state and st.session_state.is_new_user:
    st.error("""
    ### ⚠️ 필수 안내사항 - 반드시 읽어주세요!
    
    **📌 지금 즉시 이 페이지의 URL을 북마크(즐겨찾기)에 추가하세요!**
    
    귀하의 모든 독서 기록은 현재 URL에 저장됩니다.
    - 🔖 **Ctrl+D (Windows) 또는 Cmd+D (Mac)를 눌러 북마크 추가**
    - 🔗 **URL을 복사하여 메모장이나 이메일에 저장**
    - ❌ **북마크하지 않으면 다음 방문 시 기록이 모두 사라집니다**
    
    *URL 예시: `...streamlit.app/?user_id=xxxxx-xxxx-xxxx`*
    """)
    col1, col2 = st.columns(2)
    with col1:
        if st.button("✅ 북마크 완료했습니다", type="primary", use_container_width=True):
            st.session_state.is_new_user = False
            st.rerun()
    with col2:
        if st.button("📋 URL 복사하기", use_container_width=True):
            current_url = f"https://reading-log-app-g9cem8kgyigz7vkmlg3khx.streamlit.app/?user_id={user_id}"
            st.code(current_url, language="text")
            st.info("위 URL을 복사하여 안전한 곳에 저장하세요")
else:
    # 기존 사용자에게도 리마인더 표시
    with st.expander("💡 중요: 세션 저장 안내", expanded=False):
        st.warning("""
        **📌 귀하의 독서 기록을 계속 사용하려면:**
        - 현재 페이지가 북마크되어 있는지 확인하세요
        - URL의 `user_id` 파라미터가 귀하의 개인 저장소입니다
        - 이 URL을 잃어버리면 기록을 복구할 수 없습니다
        - 다른 기기에서도 동일한 URL로 접속하면 기록을 볼 수 있습니다
        """)

# 사용자 세션 정보 표시
with st.sidebar:
    st.success(f"🔐 개인 세션: {user_id[:8]}...")
    with st.expander("🆕 세션 관리"):
        st.code(f"ID: {user_id}", language="text")
        current_url = f"https://reading-log-app-g9cem8kgyigz7vkmlg3khx.streamlit.app/?user_id={user_id}"
        st.code(current_url, language="text")
        if st.button("📋 URL 복사", use_container_width=True):
            st.info("위 URL을 복사하세요")
        if st.button("🔄 새 세션 시작", help="새로운 독서 기록을 시작합니다.", use_container_width=True):
            # URL 파라미터 제거하여 새 세션 생성
            st.query_params.clear()
            if 'user_id' in st.session_state:
                del st.session_state['user_id']
            st.rerun()
        st.error("⚠️ URL 북마크 필수!\n북마크하지 않으면\n기록이 사라집니다!")
    st.markdown("---")
...
if menu == "📖 책 추가하기":
    ...
    with tab4:
        ...
        with col1:
            ...
            st.write(f"• 파일 위치: {os.path.basename(db_path)}")
...
if 'session_start' in st.session_state:
    st.sidebar.caption(f"_세션 시작: {st.session_state.session_start.strftime('%Y-%m-%d %H:%M')}_")
